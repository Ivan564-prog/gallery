...
class Diary(TimestampModelMixin, models.Model):
    STATUSES = (
        ('draft', 'Черновик'),
    ...
    held_in_diocese = models.ForeignKey(
        'local_hierarchy.Diocese', verbose_name='Проведено в епархии', on_delete=models.CASCADE, related_name='helded_diocese', null=True, blank=True)
    created_by_diocese = models.ForeignKey(
        'local_hierarchy.Diocese', verbose_name='Создано в епархии', on_delete=models.CASCADE, related_name='created_diaries', null=True, blank=True)
    ...
    advantages = models.TextField(
        verbose_name='Положительные черты', null=True, blank=True)
